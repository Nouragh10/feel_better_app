...
class FirestoreService {
  ...
  Future<String> addEntry({
    ...
    final ref =
        _db.collection('entries').doc(uid).collection(month).doc();
    await ref.set({
      'entryId': ref.id,
      'authorId': uid,
      'mood': mood,
  ...
  Future<void> notifyTrustedPerson({
    required String fromUid,
    required String fromUsername,
    required String summary,
  }) async {
    try {
      final fromUserSnap = await getUser(fromUid);
      final fromUserData = fromUserSnap.data();
      final prefs = fromUserData?['personalPreferences'] as Map<String, dynamic>?;
      final trustedPersonUsername = prefs?['trustedPerson'] as String?;
      
      if (trustedPersonUsername == null || trustedPersonUsername.isEmpty) {
        return;
      }
      
      final toUid = await getUidByUsername(trustedPersonUsername);
      if (toUid == null) return;
      
      final inbox = _db.collection('pings').doc(toUid).collection('inbox').doc();
      await inbox.set({
        'pingId': inbox.id,
        'type': 'trusted_person_notification',
        'fromUid': fromUid,
        'toUid': toUid,
      ...
      print('Failed to notify trusted person: $e');
    }
  }
  ...
  Future<void> updateUserDailyStreakOnActionComplete({
    required String uid,
    int? tzOffsetMinutes,
  }) async {
    final offset = Duration(
      minutes: tzOffsetMinutes ?? DateTime.now().timeZoneOffset.inMinutes,
    );
    final dayKey = _dayKeyWithOffset(offset);
    ...
    await _db.runTransaction((tx) async {
      ...
      final current = (data['dailyCurrentStreak'] as int?) ?? 0;
      final longest = (data['dailyLongestStreak'] as int?) ?? 0;
      ...
      if (lastDayKey == null) {
        ...
        next = consecutive ? current + 1 : 1;
      }

      tx.set(userRef, {
        'dailyLastDayKey': dayKey,
        'dailyCurrentStreak': next,
        'dailyLongestStreak': (next > longest) ? next : longest,
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
    });
  }
