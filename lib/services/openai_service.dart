...
//             prefs?['calmSong'] as String? ?? prefs?['happySong'] as String?,
...
//         case _MoodCat.low:
//           return _prefOrSearch(
//             prefs?['happySong'] as String? ?? prefs?['energizeSong'] as String?,
...
//         case _MoodCat.hungry: // already handled
//           return _webSearch('quick easy 5 minute recipe healthy snack');
//       }
//     }
...
//         i('Scan the \$item’s cover for 30 seconds while breathing in for 4, out for 6.'),
//       ],
//       'plant': [
//         i('Look closely at the \$item’s leaves for 30 seconds and count five shades of green.'),
...
class OpenAIService {
  ...
  static Future<String> suggest({
    ...
    if (apiKey == null || apiKey.isEmpty) {
      return _localSuggestion(mood, items, userPreferences);
    }

    // Online path — call OpenAI
    final uri = Uri.parse('https://api.openai.com/v1/chat/completions');
    final headers = {
      'Authorization': 'Bearer $apiKey',
      'Content-Type': 'application/json',
    ...
    if (userPreferences != null && userPreferences.isNotEmpty) {
      ...
      prefsContext += '\n\nUse these preferences when relevant. If phone is nearby and mood matches, suggest their favorite song with a search link like: "Listen to [song name] - search: https://www.google.com/search?q=[song+artist+encoded]"';
    }
...
''';
      ...
      'messages': [
        {
          'role': 'system',
          'content':
              'You return one tiny, safe action in 1–2 sentences. Be concrete, compassionate, and doable in under 2 minutes. Use user preferences when relevant.'
        },
        {'role': 'user', 'content': prompt},
      ],
    });
  ...
  static String _localSuggestion(String mood, List<String> items, Map<String, String>? prefs) {
    final thing = items.isNotEmpty ? items.first : 'something nearby';
    final m = mood.toLowerCase();
    
    // Check for phone + preferences match
    final hasPhone = items.any((i) => i.toLowerCase().contains('phone'));
    
    if (hasPhone && prefs != null) {
      if ((m.contains('sad') || m.contains('low') || m.contains('down')) && prefs['favoriteSongSad'] != null) {
        final song = prefs['favoriteSongSad']!;
        final encoded = Uri.encodeComponent(song);
        return 'Listen to "$song" - it is your go-to song when feeling low. Search: https://www.google.com/search?q=$encoded';
      }
      if ((m.contains('anx') || m.contains('tense') || m.contains('panic')) && prefs['favoriteSongAnxious'] != null) {
        final song = prefs['favoriteSongAnxious']!;
        final encoded = Uri.encodeComponent(song);
        return 'Play "$song" - your calming song. Search: https://www.google.com/search?q=$encoded';
      }
      if (prefs['favoritePodcast'] != null) {
        final podcast = prefs['favoritePodcast']!;
        return 'Listen to a 5-minute segment of "$podcast" - your favorite podcast.';
      }
    }

    // Use other preferences
    if (prefs != null) {
      if ((m.contains('low') || m.contains('sad')) && prefs['goToActivity'] != null) {
        return 'Try ${prefs['goToActivity']} for 2 minutes - your go-to activity for feeling better.';
      }
      if (prefs['comfortFood'] != null && items.any((i) => i.toLowerCase().contains('kitchen') || i.toLowerCase().contains('water'))) {
        return 'Have some ${prefs['comfortFood']} - your comfort choice. Take it slow and savor it.';
      }
    }
    ...
    if (m.contains('low') || m.contains('sad') || m.contains('down')) {
      return 'Open a page and write your full name and today date. List 3 tiny wins you can do in 2 minutes (water, window, stretch) and do the first now.';
    }
    ...
    if (m.contains('can') && m.contains('t') || m.contains('cannot') || m.contains('stuck')) {
